...
create table unified.d_genres as select name, genre_id from discog.genres;
create table unified.d_releases_genres as select release_id, genre_id from discog.releases_genres;
create table unified.d_releases as select release_id, title, released from discog.releases;
create table unified.d_releases_artists as select artist_id, release_id from discog.releases_artists;
create table unified.d_artists as select tist_id as artist_id, name from discog.artists;
...
create table unified.mb_artist_credit as select id, name from musicbrainz.artist_credit;
create table unified.mb_artist_credit_name as select artist_credit, artist from musicbrainz.artist_credit_name;
create table unified.mb_artist as select id, gender, name from musicbrainz.artist;
create table unified.mb_release as select id, name, release_group from musicbrainz.release;
create table unified.mb_medium_format as select id, name from musicbrainz.medium_format;
